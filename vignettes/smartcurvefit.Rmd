...
- Automatically estimates coefficients `a` and `b` - no starting values or manual optimization are required.  
...
The core function `fit_model()` automatically fits the chosen model type under three loss functions: **L1**, **L2**, and **Huber** - using a C++ backend for numerical optimization.  

- The function **estimates parameters `a` and `b` directly from the data**, without requiring the user to provide initial guesses.  
- Each loss function is evaluated via **5-fold cross-validation**, computing both RMSE and MAE metrics.  
- The loss with the lowest mean validation error (based on the user-selected metric) is automatically chosen.  
- Finally, the model is **refitted on the full dataset** using the chosen loss for optimal accuracy and stability.
